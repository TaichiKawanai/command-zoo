...
def ReadCommandSetting(command_list_json):
    ...
    discription = json_load["description"]
    ...
    for command_elem in json_load["commands"]:
        ...
        if "args" in command_elem.keys():
            for arg_elem in command_elem["args"]:
                key = arg_elem["arg"]
                args[key] = arg_elem["desc"], arg_elem["line"]
        cmd = command_elem["cmd"]
        commands[cmd] = command_elem["desc"], command_elem["line"], args
...
def main():
    ...
    if cmd_line and not params.argument:
        if params.show:
            print(cmd_line)
        else:
            sh_cmd = [cmd_line]
    ...
    if not params.argument:
        ...
        sys.exit(error_message)
    elif params.argument not in args.keys():
        error_message = f"{group}: '{params.argument}' is not a '{group} {params.command}' argument. See '{group} --help'.\n\n"
        error_message += "The most similar arguments are\n"
        candidates = GetSimilarOne(params.argument, args.keys())
        for arg in candidates:
            error_message += f"\t{arg}\n"
        sys.exit(error_message)

    cmd_line = args[params.argument][1]
